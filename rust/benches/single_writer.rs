...
fn benchmark_single_writer(c: &mut Criterion) {
    ...
    for (messages, producers) in [(100_000u64, 1usize), (100_000, 4), (100_000, 8), (1_000_000, 8)]
    {
        let id = format!("{}msg_{}prod", messages, producers);
        group.throughput(Throughput::Elements(messages));

        group.bench_with_input(BenchmarkId::new("agent", &id), &(messages, producers), |b, &(messages, producers)| {
            b.iter(|| {
                rt.block_on(async {
        ...
        group.bench_with_input(BenchmarkId::new("mutex", &id), &(messages, producers), |b, &(messages, producers)| {
            b.iter(|| {
                rt.block_on(async {
